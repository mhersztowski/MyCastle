...
export function elevationToColor(elev: number, minElev: number, maxElev: number, alpha = 220): RGBA {
  ...
  return [...ELEVATION_STOPS[ELEVATION_STOPS.length - 1][1], alpha] as RGBA;
}
 ...
 * Clamped to ±40%.
 */
export function slopeToColor(slopePct: number, alpha = 220): RGBA {
  const MAX = 40;
...
export function solidColor(rgb: [number, number, number], alpha = 220): RGBA {
