...
export function computeRouteStats(coords, timestamps) {
    if (coords.length < 2) {
        return {
            totalDistance: 0,
            elevationGain: 0,
    ...
    const duration = timestamps && timestamps.length >= 2
 ...
 */
export function segmentSlopes(coords) {
    const slopes = [];
    for (let i = 1; i < coords.length; i++) {
        const from = turf.point([coords[i - 1][0], coords[i - 1][1]]);
        const to = turf.point([coords[i][0], coords[i][1]]);
 ...
 */
export function routeBounds(coords) {
...
export function closestIndexByDistance(cumDist, targetKm) {
    let best = 0;
