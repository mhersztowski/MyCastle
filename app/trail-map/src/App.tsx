...
export function App() {
  ...
  // --- Route CRUD ---
  const handleAddRoute = useCallback((route: TrailRoute) => {
  ...
  useEffect(() => {
    if (!selectedRouteId) stopAnim();
  }, [selectedRouteId, stopAnim]);

  // --- Animated position (computed from animProgress + Turf) ---
  const animPosition = useMemo<[number, number, number] | null>(() => {
    if (!isAnimating || !selectedRouteId) return null;
  ...
  const handleHoverProgress = useCallback(
    (progress: number | null, coordIndex: number | null) => {
      if (progress == null || coordIndex == null || !selectedRouteId) {
        setHover(null);
        return;
      }
      const route = routes.find((r) => r.id === selectedRouteId);
      if (!route) { setHover(null); return; }
    ...
    <div style={{ display: 'flex', width: '100%', height: '100%' }}>
      ...
      <div style={{ flex: 1, position: 'relative' }}>
        ...
        {hover && (
          ...
          >
            Elevation: <strong>{hover.elevation.toFixed(0)} m</strong>
            &nbsp;&nbsp;·&nbsp;&nbsp;
            Distance: <strong>{hover.distanceFromStart.toFixed(2)} km</strong>
          </div>
        )}
      </div>
    </div>
