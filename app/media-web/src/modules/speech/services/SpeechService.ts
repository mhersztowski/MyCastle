/**
 * SpeechService — konfiguracja i wywołania TTS/STT.
 *
 * Kopia modułu z MyCastle (`iot/aura`) z jedną zmianą: konfiguracja jedzie
 * przez REST własnego backendu, a nie przez VFS po MQTT. Media to osobna
 * aplikacja i nie ma dostępu do brokera MyCastle.
 *
 * Konfiguracja leży **na serwerze**, nie w `localStorage`, bo z Kasią rozmawia
 * się i z przeglądarki, i z telefonu przez MyCastleMobile. Ustawienie głosu na
 * jednym urządzeniu ma obowiązywać na drugim — inaczej trzeba by wpisywać klucze
 * API osobno w każdym miejscu.
 */
...
const SPEECH_CONFIG_URL = '/api/kasia/glos';
...
export class SpeechService {
  ...
  async loadConfig(): Promise<SpeechConfigModel> {
    ...
    try {
      const res = await fetch(SPEECH_CONFIG_URL);
      const data = res.ok ? (await res.json()) as SpeechConfigModel : null;
      if (data) {
        this.config = {
          ...DEFAULT_SPEECH_CONFIG,
          ...data,
      ...
      console.error('Nie udało się wczytać konfiguracji głosu:', err);
      this._isLoaded = true;
      this._isLoading = false;
      return this.config;
    }
  ...
  async saveConfig(config: SpeechConfigModel): Promise<boolean> {
    ...
    try {
      ...
      console.error('Nie udało się zapisać konfiguracji głosu:', err);
      return false;
    }
  }
