...
function UserPicoSdkProjectsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setItems(projects.filter((p) => p.softwarePlatform === 'PicoSdk'));
      setError(null);
    } catch (err) {
  ...
  const handleFetchGithub = async () => {
    ...
    try {
      ...
      setGithubProjects(data.projects.filter((p) => p.softwarePlatform === 'PicoSdk'));
    } catch (err) {
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Typography variant="h4">My Pico SDK Projects</Typography>
        <Box sx={{ display: 'flex', gap: 1 }}>
          <Button variant="outlined" startIcon={<Add />} onClick={() => { setAddEmptyDialogOpen(true); setEmptyProjectName(''); }}>
            Add
          </Button>
          <Button variant="contained" startIcon={<GitHub />} onClick={() => setAddDialogOpen(true)}>
            Add From Repo
          </Button>
        </Box>
      </Box>
        ...
        {items.map((item) => (
          <Card key={item.id} sx={{ width: 220 }}>
            ...
            >
              ...
              <Chip label="RP2350 · Pico 2" size="small" sx={{ mt: 0.5 }} />
            </CardActionArea>
      ...
      <Dialog open={addEmptyDialogOpen} onClose={() => setAddEmptyDialogOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: '16px !important' }}>
          ...
          <Typography variant="body2" color="text.secondary">
            Target: Raspberry Pi Pico 2 (RP2350) · Pico SDK 2.x
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      <Dialog
        open={addDialogOpen}
        onClose={() => { setAddDialogOpen(false); setGithubProjects([]); setSelectedGithubProject(null); }}
        maxWidth="sm" fullWidth
      >
        <DialogTitle>Create Pico SDK Project from Repo</DialogTitle>
        <DialogContent>
...
export default UserPicoSdkProjectsPage;
