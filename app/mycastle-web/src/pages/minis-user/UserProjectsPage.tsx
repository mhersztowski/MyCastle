...
import { boardProfiles } from '../../modules/ardublockly2/boards/BoardProfile';
          ...
          <Card key={item.id} sx={{ width: 220 }}>
            <CardActionArea onClick={() => navigate(`/user/${userName}/project/${encodeURIComponent(item.name)}`)} sx={{ p: 1.5, pb: 0.5 }}>
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, mt: 0.5 }}>
                {item.moduleId && (
                  <Chip label={item.moduleId} size="small" color="primary" variant="outlined" />
                )}
                {item.boardProfileKey && (
                  <Chip label={item.boardProfileKey} size="small" variant="outlined" />
                )}
                {!item.moduleId && !item.boardProfileKey && (
                  <Chip label="Arduino" size="small" variant="outlined" />
                )}
              </Box>
            </CardActionArea>
