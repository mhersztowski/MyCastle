...
export function registerLoopsGenerators(gen: PygameGenerator): void {
  gen.forBlock['controls_repeat_ext'] = function (block: Blockly.Block, g: PygameGenerator): string {
    const times = g.valueToCode(block, 'TIMES', Order.NONE) || '10';
    const body = g.statementToCode(block, 'DO') || g.INDENT + 'pass\n';
    return `for _ in range(${times}):\n${body}`;
  };

  gen.forBlock['controls_whileUntil'] = function (block: Blockly.Block, g: PygameGenerator): string {
  ...
  gen.forBlock['controls_for'] = function (block: Blockly.Block, g: PygameGenerator): string {
