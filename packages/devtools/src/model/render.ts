...
export function renderMember(m: Omit<CodeMember, 'id' | 'text'>): string {
  ...
  return `${vis} ${stat}${m.name}(${params})${m.type ? `: ${m.type}` : ''}`.trim();
}
 ...
 */
export function parseMemberText(text: string): { visibility: Visibility; isStatic: boolean; name: string; type?: string; params?: CodeParam[]; kind: 'field' | 'method' } {
  let s = text.trim();
  ...
  if (paren >= 0) {
    ...
    return { visibility, isStatic, name, type, params, kind: 'method' };
  }
  const colon = s.indexOf(':');
  const name = colon >= 0 ? s.slice(0, colon).trim() : s.trim();
  const type = colon >= 0 ? s.slice(colon + 1).trim() : undefined;
  return { visibility, isStatic, name, type, kind: 'field' };
}
