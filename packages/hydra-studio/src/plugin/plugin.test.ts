...
function fakeHost() {
    ...
    const ui: HostApi['ui'] = {
        toolbar: { register(item) { state.toolbar.push(item.id); return noop; } },
        statusbar: {
            register(item) {
    ...
    const api: HostApi = {
        ...
        commands: {
            register(id, handler) { state.commands.set(id, handler); return noop; },
            async execute(id, ...args) { return state.commands.get(id)?.(...args); },
        },
        ui,
        logger: {
...
test('otwarcie pliku .hydra pokazuje interfejs obok tekstu', () => {
...
test('inne pliki wtyczki nie obchodzą', () => {
    const { api, state } = fakeHost();
...
test('rejestruje polecenia z mockupu w palecie i na pasku', () => {
    // Edytor nie ma paska menu, więc „Projekt / Buduj" trafia do palety
    // poleceń i na pasek narzędzi — pod tymi samymi nazwami.
    const { api, state } = fakeHost();
    createHydraStudioPlugin({ models: { getModel: () => undefined } }).activate(api);

    expectDeepEqual([...state.commands.keys()].sort(),
                     ['build', 'check', 'monitor', 'open', 'schematic', 'upload']);
    expectOk(state.palette.includes('hydra-studio.build'));
    expectOk(state.toolbar.includes('hydra-build'));
});

test('pasek stanu pokazuje liczbę zgłoszeń', () => {
    const { api, state } = fakeHost();
    createHydraStudioPlugin({ models: { getModel: () => undefined } }).activate(api);
...
test('polecenie budowania przekazuje robotę środowisku, nie woła pio samo', () => {
    const { api, state } = fakeHost();
    ...
    state.onOpen!('/proj/rover-01.hydra', roverSource);
    void api.commands.execute('build', 'esp32s3-main');

    expectDeepEqual(requests, [{ file: '/proj/rover-01.hydra', target: 'esp32s3-main', upload: false }]);
});

test('sprawdzanie wypisuje zgłoszenia przez dziennik edytora', () => {
    const { api, state } = fakeHost();
    createHydraStudioPlugin({ models: { getModel: () => undefined } }).activate(api);
...
test('rozszerzenie pliku jest jedną stałą, nie powtórzonym literałem', () => {
...
test('biblioteka komponentów pojawia się w bocznym panelu', () => {
    const { api, state } = fakeHost();
    createHydraStudioPlugin({ models: { getModel: () => undefined } }).activate(api);
...
test('niepowodzenie wczytania paczek nie wywraca wtyczki', () => {
    const { api, state } = fakeHost();
    createHydraStudioPlugin({
        models: { getModel: () => undefined },
...
test('płótno schematu i monitor mają własne polecenia', () => {
    // Edytor nie ma paska menu, więc „Widok / Schemat" i „Narzędzia / Monitor"
    // trafiają do palety poleceń pod tymi samymi nazwami.
    const { api, state } = fakeHost();
    createHydraStudioPlugin({ models: { getModel: () => undefined } }).activate(api);

    expectOk(state.commands.has('schematic'));
    expectOk(state.commands.has('monitor'));
    expectOk(state.palette.includes('hydra-studio.schematic'));
    expectOk(state.palette.includes('hydra-studio.monitor'));
});

test('schemat wczytywany jest razem z projektem', () => {
    const { api, state } = fakeHost();
    const asked: string[] = [];
    createHydraStudioPlugin({
        models: { getModel: () => undefined },
...
test('samo otwarcie projektu nie uruchamia pętli symulacji', () => {
    ...
    expectEqual(after, before, 'otwarcie projektu nie może zostawiać licznika');
    plugin.deactivate?.();
});
