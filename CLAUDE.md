...
- **cad-backend** — VFS REST API (`GET/POST /api/vfs/...`), multi-user `/users/{userId}/projects/{name}.cad.json`. CORS configurable (`CAD_CORS_ORIGIN`). Env: `CAD_DATA_DIR`, `CAD_BACKEND_PORT`.
- **cad-app** — React + Three.js. Cztery tryby przełączane zakładkami: **CAD** (2D/3D toggle, ortho/perspective), **CAD 3D** (parametryczny modeler z `FeatureTree` i OpenCascade.js — sketch/extrude/pocket/hole/groove/revolve/mirror/shell/loft/sweep/helix + sub-selekcja vertex/edge/face), **Scene 3D** (`RichEditor`), **Electronics** (`ComponentLibrary` + `BreadboardCanvas`).
...
- Run: `app/client/run.sh` (auto venv + deps przy pierwszym uruchomieniu).
