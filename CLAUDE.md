...
- **@mhersztowski/server-logic** (`packages/server-logic/`) — **warstwa „server logic"** (wg `docs/ServerLogic.md`), uruchamiana **in-process z mycastle-backend**. ESM-only build (tsup). Zależność tylko `@mhersztowski/minislib` (Signal/MObject); broker MQTT wstrzykiwany przez interfejs (`IMqttTransport`).
  - **`IotServer`** — mózg serwera: `log: LogService` (EnumLogKind/ILogMessage, `log/debug/info/warning/error`, `onMessage` signal, ring buffer), `activity: ActivityService` (feed zdarzeń), `console: ConsoleService`, `cron: CronService` (`every` natywnie; `schedule(expr)` przez wstrzyknięty `ICronScheduler`), `clients: ClientRegistry` (presence `(userName, device, clientType, id)`), sygnały `onServerMessage`/`onUiEvent`. `start()`/`stop()`. Opcje `broadcastLog`/`broadcastActivity`/`broadcastClients` re-publikują wpisy na `server/outbox` (`log.entry`/`activity.entry`/`clients.changed`). Komendy na `server/inbox`: `ping`→`pong`, `log`, `log.list`→`log.snapshot`, `activity.list`→`activity.snapshot`, `clients.list`→`clients.snapshot`. **W mycastle-backend broadcasty są włączone** — obsługuje je strona Server Logic.
...
- **@mhersztowski/minisc** (`packages/minisc/`) — **kompilator MinisC** (język C-podobny → bytecode `.mbc` dla MinisVM). Dual ESM+CJS build (tsup). Brak zewnętrznych zależności.
  - `src/` — pipeline kompilatora w TS: `lexer.ts` (`tokenize` → `Token[]`), `parser.ts` (`parse` → AST), `ast.ts` (typy AST + `CType`), `codegen.ts` (`codegen` → `CompiledProgram`/`CompiledFunction`), `opcodes.ts`, `natives.ts`, `packer.ts` (`pack` → `Uint8Array`, `disassemble`). Fasada `compile(source)` → `{ bytecode: Uint8Array, size, disasm }`.
  - `runtime/` — **C++ runtime jako biblioteka Arduino** (`library.properties`, `architectures=esp32`): `MinisVM.cpp/h` (stack-based VM wykonujący `.mbc`), `MinisValue.h`, `MinisNatives.h` (natywne funkcje C++ wołane ze skryptów: GPIO/MQTT/sensory), `MinisOpcodes.h`, `MinisC.h`. Skrypty wgrywane w runtime przez MQTT/OTA **bez reflashowania** (~4 KB RAM overhead). Backend wstrzykuje ten runtime do kompilacji Arduino, gdy projekt ma `useMinisC` (patrz ArduinoProject).
...
- **@mhersztowski/core-cad-viewer** (`packages/core-cad-viewer/`) — **read-only viewery scen CAD-app** wydzielone z aplikacji (reużywalne niezależnie od edytora). Dual ESM+CJS build (tsup). Peer deps: react, MUI, leaflet/react-leaflet, three/@react-three/fiber, @mhersztowski/core-cad, @mhersztowski/core-scene3d. Eksportuje 6 komponentów stron + `setViewerUserId`:
  - `pages/` — **CadViewerPage** (CAD 2D → inline SVG), **Cad3dViewerPage** (CAD → Scene3D bridge → SimpleViewer), **Scene3dViewerPage** (plik projektu Scene3D → SceneDeserializer → SimpleViewer), **ElectronicsViewerPage** (schemat → SchematicView), **MapViewerPage** (mapa Leaflet + hierarchia + opisy), **NotesViewerPage** (strony canvas).
  - **Własny minimalny VFS read-client** (`vfs.ts`: `readFileAt`, `readScene3dFile`, `base64ToText`, stałe `CAD_EXT/SCENE_EXT/ELEC_EXT/MAP_EXT/NOTES_EXT`, `setViewerUserId`) — bez funkcji zapisu; aplikacja edytora pozostaje nietknięta. Wewnętrzne moduły: `cad/buildSvg.ts` (SVG z `Project`, wymiary z grotami), `cad/cadToScene.ts`, `electronics/SchematicView.tsx` (**lekki read-only renderer SVG** z pan/zoom, bez BreadboardCanvas) + `electronics/{types,partLibrary}`, `map/{renderMapNode,InfoView,markdown,types}`, `notes/renderNotes.ts`. Używany w `app/cad-app/src/main.tsx` (route'y `/viewer/{cad,cad3d,scene3d,electronics,map,notes}/*`).
...
- Strony:
    ...
    - **Menu „Programming"** (Layout, między Electronics a IoT): pozycje **UML** → `/user/:userName/programming/uml` (full-bleed, lazy-loaded), **MinisC** → `/user/:userName/programming/minisc`, **Server Logic** → `/user/:userName/programming/server-logic` (AdminOnly, lazy). Ikony: IntegrationInstructions (grupa) + Schema (UML) + Memory (MinisC) + Dns (Server Logic).
    - **ServerLogicPage** (`pages/programming/ServerLogicPage.tsx`) — UI dla `@mhersztowski/server-logic` przez MQTT (publish `server/inbox`, subskrypcja `server/outbox` via `mqttClient.rawPublish/rawSubscribe`). Zakładki: **Overview** (status + ping latency), **Log** (live stream + wysyłka test-loga), **Activity** (feed), **Clients** (rejestr klientów), **Topics & Playground** (schemat topików + raw publish + monitor outboxa). Importuje topiki/typy/`classifyTopic`/`parseEnvelope` z pakietu (pakiet musi być zbudowany — `build:server-logic`).
    - **MiniscPage** (`pages/programming/MiniscPage.tsx`) — edytor MinisC (Monaco) z przykładowym kodem C-podobnym; przycisk Compile → `minisApi.miniscCompile(source)` (`POST /api/minisc/compile`) → pokazuje rozmiar bytecode + disassembly, Download `.mbc`. Powiązane metody `MinisApiService`: `miniscCompile(source)`, `updateProjectUseMinisC(userName, projectName, useMinisC)`.
...
- `/viewer/scene/:name` → `SceneViewerPage` — read-only viewer: ładuje `{name}.scene.json`; fallback konwersja z `.cad.json`. `SimpleViewer` z `autoFit` i `cameraPreset="cad"`. **(legacy, lokalny — nie w pakiecie)**
- `/viewer/vr/:name` → `VrViewerPage` — WebXR VR viewer, `VRCameraSetup` normalizuje skalę do 3m. **(legacy, lokalny)**
- **6 viewerów scen importowanych teraz z `@mhersztowski/core-cad-viewer`** (przeniesione z lokalnych `pages/*ViewerPage`): `/viewer/cad/*` → `CadViewerPage` (SVG), `/viewer/cad3d/*` → `Cad3dViewerPage` (bridge → SimpleViewer), `/viewer/scene3d/*` → `Scene3dViewerPage`, `/viewer/electronics/*` → `ElectronicsViewerPage` (lekki SchematicView SVG, read-only), `/viewer/map/*` → `MapViewerPage` (Leaflet + hierarchia + opisy), `/viewer/notes/*` → `NotesViewerPage`.
- `/` → `App` (główna aplikacja)

#### Unified File menu i mapy (`src/fileops/`, `src/components/`, `src/map/`)

- **`UnifiedFileMenu.tsx`** + **`fileops/FileOpsContext.tsx`** — jedno górne menu **File** wspólne dla wszystkich trybów (cad/cad3d/scene3d/electronics/map/notes), zawsze widoczne. Każdy tryb rejestruje swoje akcje (save/open/import/export + `viewerUrl`) przez `useRegisterFileOps(mode, ops, deps)`; menu czyta wpis aktywnego trybu. **Headless registrary** per tryb: `CadFileOps.tsx` (i analogiczne) hostują dialogi i rejestrują *stabilne triggery* (`register`/`unregister` z `useCallback([])` — `useRegisterFileOps` zależy od nich, **nigdy od obiektu `ctx`**, inaczej `register()`→bump `version`→nowy `ctx`→pętla renderów).
- **`map/`** — `renderMapNode.tsx` (render węzłów mapy na Leaflet), `markdown.ts` (opisy markdown/URL), `routing.ts` (trasy między punktami przez OSRM); **`RouteDialog.tsx`** (dodawanie węzła trasy: pieszo/auto/pociąg), **`InfoView.tsx`** (Compact/Fullscreen opis węzła), **`SpenNotesView.tsx`** (notatki S-Pen).
...
│   │   └── package.json
│   ├── core-cad-viewer/            # @mhersztowski/core-cad-viewer (read-only scene viewers, own VFS read-client)
│   │   ├── src/pages/              # Cad/Cad3d/Scene3d/Electronics/Map/Notes ViewerPage
│   │   ├── src/{vfs.ts,cad/,electronics/,map/,notes/}
│   │   ├── tsup.config.ts          # Dual ESM+CJS+CSS
│   │   └── package.json
│   └── minisc/                     # @mhersztowski/minisc (MinisC compiler: C-like → .mbc bytecode for MinisVM)
│       ├── src/{lexer,parser,ast,codegen,opcodes,natives,packer,index}.ts
│       ├── runtime/               # C++ Arduino lib: MinisVM.cpp/h, MinisValue/Natives/Opcodes.h, MinisC.h, library.properties (esp32)
│       ├── tsup.config.ts          # Dual ESM+CJS
│       └── package.json
│
├── app/
...
- **Build specific:** `pnpm build:core`, `pnpm build:core-backend`, `pnpm build:minisc`, `pnpm build:devtools`, `pnpm build:web-client`, `pnpm build:texteditor`, `pnpm build:web-cpp`, `pnpm build:backend`, `pnpm build:web`, `pnpm build:scene3d`, `pnpm build:core-cad`, `pnpm build:cad-viewer`, `pnpm build:cad`
- **Build CAD stack (w kolejności):** `pnpm build:cad-all` (alias `build:cadall`) = `core` → `core-cad` → `ui-core` → `core-scene3d` → **`cad-viewer`** → `ui-scene3d` → `texteditor` → `cad-backend` → `cad`. Sam `pnpm build:cad` = `core-cad` → `texteditor` → **`cad-viewer`** → `cad-app`.
- **Build MyCastle:** `pnpm build:mycastle` = `core` → `core-backend` → **`minisc`** → `devtools` → `texteditor` → `web-client` → `web-cpp` → `backend` → `web`. `dev:backend` buduje `minisc` przed startem (backend importuje z `dist`).
