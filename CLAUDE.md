...
- Komponenty (`src/components/`):
    ...
    - **GlobalDrive** — floating window wrapping DrivePage (windowName: 'drive', 1100×700) — Drive dostępny z każdej strony bez utraty stanu
...
- **Nowe extensions MdEditor** (`components/mdeditor/extensions/`):
  ...
  - **`AutomateScriptExtension`** — rozbudowany: dodano **AutomateIncludeFileDialog** (import nodes z zewnętrznego .json), **AutomateLibraryPickerDialog** (picker gotowych bibliotek węzłów), **AutomateScriptSettingsDialog** (konfiguracja skryptu)
...
- **Build specific:** `pnpm build:core`, `pnpm build:core-backend`, `pnpm build:web-client`, `pnpm build:texteditor`, `pnpm build:web-cpp`, `pnpm build:backend`, `pnpm build:web`, `pnpm build:scene3d`, `pnpm build:core-cad`, `pnpm build:cad`
