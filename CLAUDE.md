...
- **App singleton** (`src/App.ts`): `App.create(config)` → `App.instance.init()` → `App.instance.shutdown()`. Trzyma referencje do wszystkich modułów: fileSystem, ocrService, dataSource, automateService, schedulerService, httpServer, iotService, arduinoService, **upythonService**, **picoSdkService** (null gdy brak `PICOSDK_DOCKER_IMAGE`), _mqttServer (lazy), jwtService, apiKeyService, terminalService. Seeduje domyślnego admina (admin/admin) przy pierwszym uruchomieniu.
...
- **`app.config.js`** (zastąpił `app.json`) — dynamiczny config Expo czytający env vars przy buildzie: `MYCASTLE_SERVER_URL` (domyślnie `http://192.168.0.207:1894`), `MYCASTLE_APP_NAME` (domyślnie `MyCastle`). `App.tsx` czyta URL przez `Constants.expoConfig?.extra?.serverUrl`.
