...
- **@mhersztowski/web-client** (`packages/web-client/`) — reusable React client for MyCastle backend. Dual ESM+CJS build (tsup). React as peerDependency, monaco-editor as optional peerDependency.
  ...
  - `vfs/` — VFS UI components. VfsExplorer (tree browser z context menu, drag & drop wewnętrzny + **OS drag-and-drop plików i katalogów** z systemu — `webkitGetAsEntry()` + rekurencyjny upload przez `FileSystemDirectoryReader`, overlay z postępem, rozróżnienie wg `dragNodeRef.current === null`), VfsBreadcrumbs, VfsMountManager (UI do montowania providerów: MemoryFS, GitHubFS, WritableGitHubFS, BrowserFS, RemoteFS), **VfsCommitDialog** (props: `provider: WritableGitHubFS, onClose, onCommit` — pokazuje pending changes z diff view, pole commit message, commit button; `DiffView` renderuje unified diff line-by-line), useVfsTree hook, providerRegistry (memoryFsProvider, githubFsProvider, **writableGithubFsProvider**, browserFsProvider, remoteFsProvider, defaultProviderRegistry), **vfsMountPresets** (`VfsMountPreset` {id, name, mountPoint, providerType, config}; `loadPresets()`/`savePreset()`/`deletePreset()`/`generatePresetId()` — persystowane w localStorage `mycastle_vfs_presets`), getFileIcon, useVfsClipboard. `types.ts`: **`VfsProjectContext`** {id, name, platform, language (`VfsProjectLanguage` = `'MicroPython'|'Python'|'C++'|string`, derivowana przez `platformToLanguage()`), boardProfileKey?, fqbn? (custom FQBN — precedence over boardProfileKey default), projectJsonPath: string}; **`VfsExplorerProps`** rozszerzone o: `projectDeps?: ProjectDeps` (baseUrl, authToken, userName — gdy ustawione, project actions wykonują się przez built-in `Project.execute()`), `onExecuteAction?: ProjectActionExecutor` (override executor), `onDialogAction?` (actionId, context, saveProjectJson) (host odpowiada za dialog — `saveProjectJson(updates)` merguje do project.json i odświeża context), `onOutputLine?` (output linia → zewnętrzny handler, wyłącza in-sidebar panel), `onActionRunningChange?`, `hideOutput?`, `onProjectContext?` (context | null — wywoływany przy zmianie ancestor project.json), `revealPathsRef` (wywoływany po agent write — expand & scroll do plików). **`vfs/project/`** — **system projektów embedded**. `Project` (klasa bazowa): `getTypeLabel()`, `getActions(): ProjectAction[]`, `execute(actionId, selectedPath, onOutput, signal)`, helpers `apiPost()`, `apiGetSSE<T>()` (SSE streaming), `deriveSketchName(selectedPath)`, `promptPort()`. `types.ts`: `ProjectAction` {id, label, description?, hasOutput, hasDialog?, shortcut?}, `ProjectDeps`, `ProjectActionExecutor`, `OutputLine` {text, timestamp, type: 'normal'|'error'|'warning'|'success'|'command'}, `classifyLine(text)` — heurystyczna klasyfikacja linii wyjścia. **`ArduinoProject`**: actions Compile (F7)/Flash (F8)/Build&Flash/Clean/Board… (hasDialog); `BOARD_FQBN` map: uno/nano_328/mega/leonardo/esp8266_huzzah/esp8266_wemos_d1/esp32_devkitc/esp32s3_devkitc/esp32s3_pico/esp32s3_zero; custom `fqbn` z project.json ma precedence; `deriveSketchName()` wyznacza sketch z wybranej ścieżki. **`NodeJsProject`**: actions install/build/dev/start/test; stripuje prefix montowania VFS (`/home/{userName}/` lub `/home/`) żeby uzyskać `subpath` → `GET/SSE /api/users/{u}/nodejs/run?subpath=&script=`.
  - `monaco/` — Monaco Editor wrapper. `core/`: EditorInstance, ModelManager, CommandRegistry, EventEmitter. `language/`: LanguageService, FormattingService, C++ plugin. `plugins/` — **kompletny system pluginów v2**: `types.ts`: `IPluginManifest` {id, name, version, description?, contributes: ContributionPointType[]}, `ContributionPointType` ('toolbar'|'statusbar'|'contextmenu'|'commandpalette'|'sidebar'), `ToolbarContribution` {id, label, icon, group?, order?, when?, command, toggle?}, `IStatusBarItemHandle` {update(patch), dispose()}, `StatusBarContribution`, `ContextMenuContribution`, `CommandPaletteContribution`, `SidebarContribution` {id, title, component: React.ComponentType}, `IPluginAPI` z namespace-ami: `editor` (onDidOpenDocument, onDidChangeContent, onDidChangeModel, openVirtualTab), `commands` (register, execute), `ui` (toolbar, statusbar, contextmenu, commandpalette, sidebar), `events` (on/emit — auto-prefixed pluginId), `storage` (get/set localStorage), `logger` (info/warn/error). `EventBus.ts`: singleton `globalEventBus` — pub/sub między pluginami i edytorem; system events `system:editor:modelChanged/contentChanged/saved`, `ui:toolbar:changed`, `ui:statusbar:changed` etc. `PluginCommandRegistry.ts`: `globalCommandRegistry` — rejestr komend pluginów (odrębny od Monaco actions). `UIRegistry.ts`: `globalUIRegistry` — osobne rejestry dla toolbar/statusbar/contextmenu/commandpalette/sidebar + React hooks `useToolbarItems()`, `useStatusBarPluginItems()` etc. `PluginRegistry.ts`: `globalPluginRegistry` — lifecycle (register/activate/deactivate), stany inactive→activating→active→deactivating; `defineEditorPlugin()` helper z pełną inferencją typów. `PluginAPI.ts`: `createPluginAPI()` — per-plugin scoped API, auto-tracks disposables, integracja z globalEventBus/globalCommandRegistry/globalUIRegistry. `state/`: EditorStateManager. `ui/`: ContextMenuService, DecorationManager, StatusBar. `utils/`: types, disposable (DisposableStore), debounce. **MonacoMultiEditor** — VS Code-like component: Activity Bar + Sidebar (VFS file browser) + tabbed multi-editor z Split Editor + Menu Bar + Status Bar. Plugin activation on mount. Virtual editor tabs (React component jako tab — bez backing VFS file). Toolbar i statusbar items z pluginów renderowane przez UIRegistry hooks. Events: `system:editor:modelChanged/contentChanged/saved` emitowane do globalEventBus. Ctrl+S save. monacoWorkers.ts w consuming app. `agent/`: **AgentEngine** — `run(prompt, vfsProvider?)`, `abort()`, `loadHistory(messages)`. `types.ts`: `ChatAttachment`, `ChatSession`, `AiChatMessage.attachments?`, `AiProviderConfig.signal?`.
...
- **App singleton** (`src/App.ts`): `App.create(config)` → `App.instance.init()` → `App.instance.shutdown()`. Trzyma referencje do wszystkich modułów: fileSystem, ocrService, dataSource, automateService, schedulerService, httpServer, iotService, arduinoService, **upythonService**, **picoSdkService** (null gdy brak `PICOSDK_DOCKER_IMAGE`), **pluginService** (web), **backendPluginService**, _mqttServer (lazy), jwtService, apiKeyService, terminalService. Seeduje domyślnego admina (admin/admin) przy pierwszym uruchomieniu. `init()` woła `backendPluginService.loadAllUsers()` po starcie HTTP; `shutdown()` woła `backendPluginService.shutdownAll()`.
...
- **MdEditor** — `BlockActionMenu`: jeden przycisk na blok (pozycje obliczane przez `updateBlockPositions()` — sync ProseMirror node attrs do DOM dla NodeView-based bloków). `SlashCommands` otrzymuje `onCreatePage` callback (ref pattern dla stabilności). `BlockIdExtension` dołączony bezpośrednio w MdEditor. **SlashCommands** — nowe komendy: `Link` (prompt URL + tekst, wstawia link), `Page` (prompt ścieżka, tworzy plik .md + wstawia link), **`Plugin Script`** (wstawia pusty wykonywalny blok skryptowy) oraz **dynamiczne pozycje `Plugin Script: {label}`** — po jednej na każdy szablon zarejestrowany przez załadowane web-pluginy (`buildPluginTemplateCommands()` czyta `pluginRegistry.getTemplates()` przy każdym zapytaniu `/`; opis pozycji zawsze zaczyna się od nazwy źródłowego pluginu; wybór wstawia `template.code` z `template.mode`/`label`). `PluginScriptBlock` dołączony do extensions MdEditora. **WorkspaceMdPage** — `handleCreatePage(path)`: tworzy plik przez `writeFile`, inkrementuje `treeVersion` do odświeżenia drzewa; lista plików pokazuje folder jako secondary text.
...
- `src/components/ActionBar.tsx` — górny poziomy pasek (nad canvasem): Transform (Move/Copy/Rotate — wymagają wcześniejszego zaznaczenia), Edit (Offset/Trim/Fillet/Dimension), Undo/Redo/Delete. Wydzielony z dawnego `Toolbar`.
- `src/components/GripOverlay.tsx` — nakładka uchwytów (grips) dla zaznaczonych encji w trybie 2D/select: kwadratowe punkty na charakterystycznych miejscach (line: końce; rect: rogi; circle: środek + 4 kwadranty; arc: środek/start/end…), przeciąganie uchwytu edytuje geometrię.
- `src/components/ScaleBar.tsx` — pasek skali w rogu canvasu 2D: wyznacza „ładną" wartość z listy `NICE` najbliższą ~80px, aktualizowany przez `renderer.onViewChange`.
...
- `src/components/StatusBar.tsx` — dolny pasek: aktywne narzędzie + hint, aktywna warstwa, liczba encji/zaznaczonych, tryb widoku, **`GridInput`** — edytowalna inline wartość rozmiaru siatki (`project.settings.gridSize`).
